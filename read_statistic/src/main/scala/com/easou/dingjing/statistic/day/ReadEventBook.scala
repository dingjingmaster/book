...
object ReadEventBook {
  def main(args: Array[String]): Unit = {
    val bilogPath = args(0)
    val savePath = args(1)

    val conf = new SparkConf().setAppName("day_statistic_book")
  ...
  def statistic(x: Tuple2[String, List[String]]): String = {
    var gids = scala.collection.mutable.Set[String]()

    for (gid <- x._2) {
      gids.add(gid)
    }

    return "" + x._1.replace("|", "\t") + "\t" + gids.toArray.length.toString
  }
  ...
  def parse_log(x : String) : Tuple2[String, List[String]] = {
    ...
    return (appid+"|"+bookStatus+"|"+bookType, List(gid))
  }
}